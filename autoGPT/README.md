...
```bash
# 在仓库根目录执行（依赖由根目录 pyproject.toml 管理）

# 1. 安装依赖（首次）
uv sync

# 2. 下载数据并训练分词器（一次性，约 2 分钟）
uv run autoGPT/prepare.py

# 3. 手动运行一次训练实验（约 10 分钟）
uv run autoGPT/train.py
```
...
```
请阅读 autoGPT/program.md，我们开始一轮新实验！先完成初始化设置。
```
...
tests/
  test_train_config.py  — train.py 配置合法性校验
...
- **固定时间预算。** 训练始终运行 10 分钟，与具体计算平台无关。不同实验之间均可直接比较；同时也意味着 autoresearch 会在该时间预算内为你的平台找到最优模型。但注意，不同计算平台之间的结果不具可比性。
